...
feature_type = pd.read_csv(features_type)
feature_type.index = feature_type.feature
feature_type = feature_type.drop('feature',axis=1)
feature_type = feature_type.to_dict()['type']
...
for feature in features:
    ...
    n_null = len(train_x[train_x[feature]<0])
    feature_info[feature] = [max_,min_,n_null]
...
test_result.to_csv('../result/test_result_xgb.csv',index=None,encoding='utf-8')

#save feature score and feature infomation
feature_score = model.get_fscore()
for key in feature_score:
    feature_score[key] = [feature_score[key]] + feature_info[key] + [feature_type[key]]
...
for (key,value) in feature_score:
    fs_key.append(key)
    fs_score.append(value[0])
    fs_max.append(value[1])
    fs_min.append(value[2])
    fs_n_null.append(value[3])
    fs_type.append(value[4])
feature_score_csv = pd.DataFrame(columns=['feature','score','max','min','n_null','type'])
feature_score_csv.feature = fs_key
feature_score_csv.score = fs_score
feature_score_csv.max = fs_max
feature_score_csv.min = fs_min
feature_score_csv.n_null = fs_n_null
feature_score_csv.type = fs_type
feature_score_csv.to_csv('../data/raw_feature_score.csv',index=None,encoding='utf-8')
